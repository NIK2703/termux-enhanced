...
Правило (одно предложение): **пока `isFlingActive()` — ось жеста привязана к тому концу, к которому он летит: в историю — сдвигаем обе стороны на одинаковое число строк, к живому низу — не компенсируем вовсе.**

Реализация (`TerminalView`, сделано):

- `anchorFlingToContent(int rowShift)` вызывается из ветки fling в `onScreenUpdated(boolean)` (раньше там не было ничего). Для fling **в историю** (`mFlingRawVelocity > 0`) сдвиг применяется дважды: `mTopRow -= applied` и `mFlingAnchorShiftPx -= applied * rowHeight`. В `runFlingFrame()` позиция читается как `posPx = mScroller.getCurrY() + mFlingAnchorShiftPx`, поэтому `diff = newRow - mTopRow` — единственное, что fling реально применяет к позиции, — не меняется: ни джиттера, ни «плывущего» текста, ни отмены follow-text.
- Для fling **к живому низу** компенсации нет: низ — это «сейчас», он не должен уезжать (`mTopRow == 0` остаётся актуальным и без компенсации).
- Снимок `minPx/maxPx` на старте жеста остаётся валидным, потому что в сдвинутой оси самая старая строка всегда лежит на `-T0·rowHeight`. Перезапускать scroller (и трогать физику/длительность) не требуется.
- Остаток (`mScrollRemainder` в `captureFlingRemainder()`) и проверки границ/glow в `runFlingFrame()` считаются по `posPx`, а не по сырому `mScroller.getCurrY()` — иначе после сдвига они снова сравнивали бы разные системы координат.

> **⚠ Ошибка прежней версии этого раздела.** Здесь утверждалось: «если лимит не достигнут и история растёт — `minPx` устаревает, что **безопасно**: fling просто не дотянется до новых строк, следующий жест пересчитает диапазон». Это неверно, и ровно это наблюдалось как баг.
>
> `mTopRow` — **адрес** в пространстве, чей ноль («сейчас») привязан к живому низу, а не координата содержимого. Каждая напечатанная строка перенумерует пространство: `TerminalBuffer.scrollDownOneLine()` делает `mActiveTranscriptRows++`, и та же самая старая строка получает адрес на 1 глубже. Ось жеста при этом зафиксирована на старте (`minPx = -getActiveTranscriptRows() * rowHeight`), а `SplineOverScroller.fling()` клампит `mFinal` по этому `minPx` и укорачивает длительность (`adjustDuration`). За время полёта напечатано N строк ⇒ реальный верх истории ушёл на N строк глубже, ось стоит на месте ⇒ **недолёт ровно N строк**. Упор жёсткий и без обратной связи: `edge-glow`, поглощение и обрезка хвоста сравнивают позицию с *живой* границей, а стенка — фантомная (из устаревшего `minPx`), поэтому они молчат.
>
> Инвариант после фикса: **свайп покрывает столько же строк содержимого и доезжает до того же конца, что и в молчащем терминале** (`terminal-emulator/src/test/java/com/termux/terminal/FlingLiveOutputTest.java`: `testFlingIntoHistoryFallsShortByPrintedLines` — N = 7 с выключенным фиксом; `testAnchoredFlingIntoHistoryReachesTheTop`, `testAnchoredFlingCoversTheSameDistance`, `testFlingTowardsBottomStillReachesIt`, `testFullTranscriptHasNoShortfall` — с включённым).
