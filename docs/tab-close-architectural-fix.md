...
```java
/** ПРАВАЯ соседка (та, что визуально встаёт на освободившееся место), иначе левая.
...
Направление (правая, левая — только когда закрыли последнюю вкладку) — сознательное;
обоснование и замеры в §5-J. Обратите
...
### J. Пейджер уводится с удаляемой страницы ДО удаления; наследник — правая соседка
...
2. `pickHeir()` предпочитает **правую** соседку (левую — только если закрыли последнюю
   вкладку). Вкладки 1,2,3,4, закрываем 3 → 4; закрываем 4 → 3. Это исходное намерение проекта
   (`tab-close-stuck-in-dead-session.md`: «переехать в её слот — правый сосед»), но до перехода
   на сессионную цель оно никогда не достигалось: синк работал с «индексом в старом списке» и
   клампил его, а раскладка пейджера выигрывала — на экране всегда оказывалась левая соседка.
   Теперь цель — сессия, её индекс резолвится после удаления, поэтому политика реально доезжает
   до экрана.

   Цена: наследница стоит на `removedIndex+1` до удаления и на `removedIndex` после, поэтому
   парк уводит пейджер на одну страницу **дальше** финальной цели, а синк затем отступает на одну
   страницу назад (`setCurrentItem(removedIndex, false)` — реальный ход, а не no-op). Ровно та же
   форма, что у сценария «закрыть первую вкладку», который измерен и приземляется корректно
   (§6): парку важен только факт ухода с обречённой страницы, а не точка приземления.
   При закрытии **последней** вкладки работает левый фолбэк — там индекс наследницы не
   сдвигается и `setCurrentItem(restoreIndex, false)` остаётся no-op.
...
> **Прогоны выше сняты при политике «левая соседка»** (шаг 7, первая редакция §5-J). После
> переворота на «правую соседку» (19.09) ожидаемые точки приземления меняются: закрытие 3-й из
> 4 даёт `pager=2 active=2` (вкладка 4), закрытие 2-й из 3 — `pager=1` (вкладка 3). Инварианты
> (нет `pos=-1`, подсветка == экран) переносятся без изменений, но **сами строки таблицы надо
> переснять**; до этого они — исторический след, а не текущий оракул.
...
| 7 | (J) пре-парк + правая соседка | закрытие любой вкладки: `child pos=0 x=0` — это наследница; ни одного `pos=-1` |
