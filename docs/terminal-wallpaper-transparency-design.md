...
### 6.3 Прозрачим только фон «по умолчанию»

Ячейки с явно заданным фоном (`ls --color`, `htop`, подсветка синтаксиса) **остаются непрозрачными**. Причины:

1. Программа, явно выставившая цвет фона, хочет видеть именно его — это часть её разметки.
2. Читаемость: при 50% прозрачности цветные блоки поверх обоев «плывут»; непрозрачные — нет.
3. Это даётся **бесплатно**: `TerminalRenderer.java:371` уже пропускает такие ран-блоки (`if (backColor == palette[COLOR_INDEX_BACKGROUND]) continue;`), так что за них рисовать не надо вообще.

Побочный эффект (осознанный): в `htop` цветные полосы останутся плотными, а фон вокруг — прозрачным. Это выглядит естественно.
...
> **Ничего больше в рендерере менять не нужно.** Проверено по коду:
> * pass A (строки 368-386) — пропускает фон по умолчанию, остальное рисует поверх, `SRC_OVER` верен;
> * pass B / mismatch-runs (строки 485-490) — то же условие `backColor != palette[…BACKGROUND]`;
> * курсор и текст — рисуются `SRC_OVER` непрозрачными цветами, на прозрачность не влияют.
