...
`pickHeir()` (`TermuxTerminalSessionActivityClient:1222`) выбирает для этого **правую соседку**
закрытой вкладки (левую — только если закрывали последнюю; политика перевёрнута 19.09, до этого
была левая, см. `tab-close-architectural-fix.md` §5-J). Но это решение нужно ровно в одном
...
1. `:1157` — `final TerminalSession heir = pickHeir(service, finishedSession);` → вкладка **3**
   (правая соседка закрытой; при прежней левой политике была бы 1). Считается **всегда**, без
   условий.
2. `:1168` — парк:
